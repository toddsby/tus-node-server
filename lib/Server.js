...
class TusServer extends EventEmitter {
    ...
    handle(req, res) {
        ...
        res.set('Tus-Resumable', TUS_RESUMABLE);
        if (req.method !== 'OPTIONS' && req.headers['tus-resumable'] === undefined) {
            //res.writeHead(412, {}, 'Precondition Failed');
            res.status = 412;
            res.set('Tus-Resumable','Precondition Failed');
            return res.body = { message: 'Tus-Resumable Required' };
        ...
        if (invalid_headers.length > 0) {
            ...
            res.set('Tus-Resumable','Precondition Failed');
            //res.writeHead(412, {}, 'Precondition Failed');
            return res.body = { message: `Invalid ${invalid_headers.join(' ')}` };
